  ...
  const FilterSidebar = ({ isMobile = false }) => (
    <div className={isMobile ? "h-full overflow-y-auto" : ""}>
      <div className="flex items-center justify-between mb-6">
        ...
        {isMobile && (
          <button onClick={() => setShowMobileFilters(false)} className="text-[#5a6380] hover:text-[#1e2a5e]">
            <X className="w-5 h-5" />
          </button>
        )}
      </div>
      ...
      <div className="mb-6">
        ...
        <div className="flex items-center gap-3">
          <Checkbox id="in-stock" checked={inStockOnly} onCheckedChange={(checked) => setInStockOnly(checked as boolean)} className="rounded-none border-[#8fa0d8]" />
    ...
    <div className="overflow-x-hidden bg-[#f7f8fc]">
      ...
      <section className="relative overflow-hidden py-20 bg-[#eef0f8]">
        <div className="container">
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div>
              <p className="text-[10px] font-body font-medium uppercase tracking-[0.3em] text-[#2d3a8c] mb-4">Our Collection</p>
              <h1 className="font-display text-5xl md:text-6xl text-[#1e2a5e] leading-[1.1] mb-4">All Products</h1>
              <p className="text-sm font-body font-light text-[#5a6380] max-w-md leading-relaxed">
                Discover our curated skincare collection — clean formulas, 100% natural ingredients, dermatologically tested and cruelty-free.
              </p>
            </div>
            <div className="hidden lg:grid grid-cols-3 gap-3 h-[240px]">
              {LOCAL_IMGS.map((src, i) => (
                <div key={i} className="overflow-hidden">
                  <img src={src} alt={`Product ${i + 1}`} className="w-full h-full object-cover hover:scale-105 transition-transform duration-700" />
                </div>
              ))}
            </div>
          </div>
              ...
              <>
                <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
                  {paginatedProducts.map((p, idx) => {
                    ...
                    const fallback = LOCAL_IMGS[idx % 3];
                    return (
                      <div key={p.id} className="group">
                        <div className="relative aspect-[3/4] overflow-hidden bg-[#dde8f8]">
                          {p.image ? (
                            ...
                            <img src={fallback} alt={p.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" />
                          )}
      ...
      <section className="py-14 bg-[#eef0f8]">
        <div className="container">
          ...
          <div className="mt-10 pt-8 border-t border-[#dde2f0] flex flex-wrap gap-6 justify-center text-[11px] font-body font-light text-[#5a6380]">
            <span className="font-medium text-[#1e2a5e] uppercase tracking-[0.15em]">Need Help?</span>
            <a href="mailto:hello@sleetcare.com" className="hover:text-[#1e2a5e] transition-colors">✉ hello@sleetcare.com</a>
            <a href="tel:+923008662833" className="hover:text-[#1e2a5e] transition-colors">✆ +92 300 8662833</a>
            <a href="/contact" className="hover:text-[#1e2a5e] transition-colors">→ Contact Us</a>
          </div>
        </div>
      </section>
    </div>
